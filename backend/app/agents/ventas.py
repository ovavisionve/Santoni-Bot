...
"""
    ...
    def fetch_data(self, message: str, org_ids: list[int] | None = None, salesrep_id: int | None = None) -> str | None:
        ...
        if any(w in msg for w in ["top", "mejor", "ranking", "pareto", "principales"]):
            ...
            data = build_top_clients(limit=limit, zona=zona, vendedor=vendedor, anio=anio, org_ids=org_ids, salesrep_id=salesrep_id)
            sections.append(f"## Top {limit} Clientes por Ventas ({label})")
            sections.append(self._format_table(data))

        if any(w in msg for w in ["cobran", "cobro", "recauda", "pago"]):
            data = build_collection_summary(zona=zona, vendedor=vendedor, mes=mes, anio=anio, org_ids=org_ids, salesrep_id=salesrep_id)
        ...
        if any(w in msg for w in ["venta", "factur", "ingreso", "volumen"]) or not sections:
            data = build_sales_summary(zona=zona, vendedor=vendedor, mes=mes, anio=anio, org_ids=org_ids, salesrep_id=salesrep_id)
